...
const logErr = err => console.error('error loading scripts:', err);
...
window.ramdaRepl = function ramdaRepl(targetEl, config) {
  ...
  const loadRamda   = loadScript({ src: config.ramdaUrl })
  const loadScripts = sequence(Future.of, config.scripts.map(loadScript));

  loadRamda.chain(loadScripts).fork(logErr, () => {
